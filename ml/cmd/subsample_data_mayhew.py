""" Given data files in standard format, subsample them by first subsampling surface-mention groups. 
...
def run(args):
    ...
    for i, d in enumerate(data):
        for a in d["gold_annotations"]:
            ...
            types.add(a["type"])
    nann = lambda annotations_by_mention: sum(len(ms) for ms in annotations_by_mention.values())
    ...
    logger.info(
        ...
        f"\n  => targeting {target_n} true annotations left w/ {int((1-args.precision)*target_n)} additional fps"
    )
    ...
    while fp_count < n_fp:
        ...
        observed_idxs = {i for a in d["gold_annotations"] for i in range(a["start"], a["end"])}
        unobserved_idxs = list(set(range(len(d["tokens"]))) - observed_idxs)
        if unobserved_idxs:
            s = int(npr.choice(unobserved_idxs, size=1)[0])
            e_choices = [e for e in range(s + 1, min(len(d["tokens"]), s + 4)) if e not in observed_idxs]
            if e_choices:
                e = int(npr.choice(e_choices, size=1)[0])
                t = str(npr.choice(list(types), size=1)[0])
                m = " ".join(d["tokens"][s:e])
                fp = dict(kind=args.kind, type=t, start=s, end=e, mention=m, comment="False Positive")
    ...
    logger.info(
        f"Targeting {target_n} true annotations, {int((1-args.precision)*target_n)} additional fps. Got {recall_n} and {fp_count}."
    )

    # Sort the annotations by start and write them out
    for d in psl_data:
        d["gold_annotations"] = sorted(d["gold_annotations"], key=lambda a: a["start"])
    with open(f"{outprefix}_r{args.recall:1.1f}_p{args.precision:1.1f}.jsonl", "w") as outf:
        for d in psl_data:
